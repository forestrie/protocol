...
| `vectors/` | Conformance vectors: the [grant and leaf format](vectors/grant-and-leaf-format.md) with its cross-language vectors, the [checkpoint receipt KAT-39](vectors/checkpoint-receipt-format.md) (tree, size-driven fold, rejected shapes, protected-header classes, signed receipts), and the golden receipt set. `SHA256SUMS` pins every file |
