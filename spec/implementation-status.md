...
| Component | What it does | What the specification says | Tracked |
|---|---|---|---|
| Consistency fold and signed size — deployments | The contract source since [univocity v0.3.0](https://github.com/forestrie/univocity/releases/tag/v0.3.0) takes every proof's base from anchored state, pins the proof shape to its sizes, and requires the signed `tree-size-2` ([univocity #42](https://github.com/forestrie/univocity/pull/42), [#43](https://github.com/forestrie/univocity/pull/43)). Instances deployed from earlier source accepted a base-zero first proof folded from the caller's peaks and did not check a signed size; the operator's estate was reset and its instances redeployed from v0.3.0 rather than migrated | [trust-boundaries-and-operator-powers.md](./trust-boundaries-and-operator-powers.md) §4.1 and [checkpoints-and-receipts.md](./checkpoints-and-receipts.md) §1.3 describe v0.3.0. A forest's genesis document names the instance it anchors to; a relying party checking a pre-v0.3.0 instance does not have the §4.1 guarantee against a key holder | [ADR-0066](../decisions/adr-0066-sec-signed-checkpoint-size.md) |
